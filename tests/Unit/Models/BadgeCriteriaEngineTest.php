...
class BadgeCriteriaEngineTest extends TestCase
{
    ...
    {
        $badge = $this->makeBadge([
            'library_size' => ['min' => 5, 'max' => 30],
        ]);

        $this->assertTrue($badge->evaluateCriteria(['library_size' => 10]));
        $this->assertFalse($badge->evaluateCriteria(['library_size' => 3]));
        $this->assertFalse($badge->evaluateCriteria(['library_size' => 31]));
    }
            ...
            'conditions' => [
                ...
                [
                    ...
                    'conditions' => [
                        ...
                        ['stat' => 'library_size', 'operator' => 'between', 'value' => [5, 10]],
                    ],
                ],
            ],
        ]);

        $this->assertTrue($badge->evaluateCriteria(['books_completed' => 5, 'current_streak' => 0, 'library_size' => 7]));
        $this->assertFalse($badge->evaluateCriteria(['books_completed' => 5, 'current_streak' => 0, 'library_size' => 11]));
        $this->assertFalse($badge->evaluateCriteria(['books_completed' => 4, 'current_streak' => 30, 'library_size' => 7]));
    }
