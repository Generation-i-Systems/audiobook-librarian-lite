...
@section('content')
    <div class="container">
        ...
        <h2 class="mt-4">Titles with this tag</h2>
        <ul class="list-group">
            @forelse($titles as $entry)
                <li class="list-group-item">
                    {{ $entry['title'] }}@if($entry['author']) — {{ $entry['author'] }}@endif
                </li>
            @empty
                <li class="list-group-item text-muted">No titles currently have this tag.</li>
            @endforelse
        </ul>
    </div>
@endsection
